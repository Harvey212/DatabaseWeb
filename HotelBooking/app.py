...
# Config MySQL
app.config['MYSQL_HOST'] = 'localhost'#'140.122.184.121'
app.config['MYSQL_USER'] = 'root'#'team13'
app.config['MYSQL_PASSWORD'] = '60808015e'#'Q1H1Fe'
app.config['MYSQL_DB'] = 'final_test'#'team13'
...
def member_account():
    ...
    # Execute query
    _SQL = """SELECT name, email, phone_num, payment
              FROM customer
              WHERE customer_id = %s"""
    ...
    return render_template('member_account.html', 
                            form = form,
                            customer = customer)
